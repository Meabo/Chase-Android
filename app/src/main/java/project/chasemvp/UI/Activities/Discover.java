...
import project.chasemvp.Model.POJO.Discover.Locationarea;
...
import project.chasemvp.UI.Interfaces.View.DiscoverViewInterface;
import project.chasemvp.Utils.CircleTransform;
import project.chasemvp.Utils.CustomTextView;
import project.chasemvp.Utils.Dialogs.AreaDialog;
...
public class Discover extends AppCompatActivity implements DiscoverViewInterface,
        ...
        View.OnClickListener, ServiceConnection
{
    ...
    @BindView(R.id.discover_constraint_layout)
    ConstraintLayout discover_cs;
    @BindView(R.id.mapview)
    MapView mapView;
    @BindView(R.id.discover_user) ConstraintLayout discover_user;
    @BindView(R.id.map_button_itinary)
    FloatingActionButton map_button_itinary;
    @BindView(R.id.map_button_catch) FloatingActionButton map_button_catch;
    @BindView(R.id.map_user_image)
    CircularImageView map_user_image;
    @BindView(R.id.map_user_xp)
    ProgressBar map_user_xp;
    @BindView(R.id.discover_level_value)
    CustomTextView map_user_level;
    @BindView(R.id.toolbar)
    Toolbar toolbar;

    CustomTextView map_current_time;
    CustomTextView map_distance_value;
    CustomTextView map_score_value;
    CustomTextView map_current_speed;
    CustomTextView map_my_pseudo;
    CircularImageView map_my_image;
    ProgressBar map_my_progress;
 ...
 */
    ...
    private void InitScreen()
    {
        ...
        map_user_xp.setMax(pref.get("xp_max", 0));
        map_user_xp.setProgress(pref.get("xp", 0));
        map_user_level.setText(pref.get("level", 0) + "");
    }



    @OnClick(R.id.map_user_image)
    public void onclick()
    {
        if (servicegps.getmyloc()!= null)
            animatecamera(servicegps.getmyloc());
    }
    ...
    public void onStart()
    {
       ...
       // startService(new Intent(this, LocationTrackingService.class));
        ...
        Log.d(debug,"onStart in Discover");
    }
...
*/
    ...
    public void onEventMainThread(SucessEvent event) {
        if (event.isSuccess())
        {
            Marker m = ChasesOnMap.get(current_area);
            m.setVisible(false);
            if (line != null)
                line.remove();
            /*
    ...
    {
        Log.d(debug, "Distance is " + socketEventDistance.getDistance());
    ...
    public void setMap_button_catch(View view) {
        switch (view.getId()) {
            case R.id.map_button_itinary:
                ...
                locationPresenter.calculateitinary(object.getPosition());
                map_button_itinary.setVisibility(View.GONE);
                break;
            case R.id.map_button_catch:
                locationPresenter.getSocket().emit("calculatedistance", locationPresenter.getmCurrentLocation().getLatitude(), locationPresenter.getmCurrentLocation().getLongitude());
                break;
            default:
                break;
        }
